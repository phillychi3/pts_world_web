<script>
    ...
    async function saveChanges() {
        ...
        try {
            const response = await fetch(`/api/guild/${formatChannelId(guild.guild_id)}/role-dropdown`, {
                method: 'POST',
                headers: {
...
<div class="bg-white rounded-lg shadow-md p-6">
                                    ...
                                    <table class="min-w-full bg-white">
                                        <thead>
                                            <tr class="bg-gray-100 border-b">
                                                <th class="text-left py-2 px-3 text-xs font-medium text-gray-700">表情符號</th>
                                                <th class="text-left py-2 px-3 text-xs font-medium text-gray-700">標籤</th>
                                                <th class="text-left py-2 px-3 text-xs font-medium text-gray-700">身分組 ID</th>
    ...
    {#if isAdding || (editingDropdown && editingRoleIndex === -1 && !isAddingRole)}
        <div class="bg-gray-50 p-6 rounded-lg border border-gray-200 mb-6">
            <h3 class="text-lg font-medium mb-4">{isAdding ? '新增身分組下拉選單' : '編輯身分組下拉選單'}</h3>
        ...
        <div class="bg-gray-50 p-6 rounded-lg border border-gray-200 mb-6">
            ...
            <div class="mb-4">
                ...
                <input
                    ...
                    class="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                />
                <p class="text-xs text-gray-500 mt-1">選項旁顯示的表情符號，例如 `:smile:` 或自訂表情符號 (例如 `:custom:1234567890`)</p>
            </div>

            <div class="flex justify-end space-x-3 mt-6">
                <button
                    class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
                    on:click={() => { isAddingRole = false; editingRoleIndex = -1; }}
                >
                    取消
                </button>
                <button
    ...
    <div class="mt-8 border-t border-gray-200 pt-4">
        ...
        <p class="text-gray-600">
            身分組下拉選單功能允許伺服器成員通過下拉選單選擇自己想要的身分組。
        </p>
    </div>
</div>